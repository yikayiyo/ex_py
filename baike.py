...
def main():
    ...
    data = []
    kwd = raw_input('关键词：')
    filename = '{}{}'.format(kwd,'.txt')
    page_nums = 10
    
    for i in range(page_nums):
        # print '下载第{}页'.format(i+1)
        count = 10*i
        url_base = 'https://www.baidu.com/s?wd={kwd}&pn={count}&tn=SE_baiduxueshu_c1gjeupa&ie=utf-8&filter=%28sc_c0%3A%3D%7B36%7D%29&sc_f_para=sc_tasktype%3D%7BfirstSimpleSearch%7D&sc_hit=1'.format(
            kwd=kwd, count=count)
        # print url_base
